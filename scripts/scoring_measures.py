...
from simple_model.model import get_device


def irof_score(model, image, attribution, raw_image, label, n_segments, sigma, batch_size):
    color_channels, width, height = image.shape
    # Baseline = mean per color channel
    base_color = torch.mean(image.view(image.shape[0], -1), dim=1).reshape(color_channels, -1)

    # Compute segments and mean relevancy per segment
    segments = slic(raw_image.numpy().transpose(1, 2, 0), n_segments, sigma).reshape(-1)
    segment_mean_relev = np.asarray([np.mean(attribution.reshape(-1)[segments == seg]) for seg in range(n_segments)])
    ...
    original_prob = calculate_probs(model, label, image.reshape(-1, image.shape[0], image.shape[1], image.shape[2]))[0]

    temp_image = image.clone().reshape(color_channels, -1)
    new_probs = np.zeros(len(segment_mean_relev))

    # Calculate new probs. batch-wise
    new_images = torch.zeros((batch_size, color_channels, width*height), device=get_device())
    for i, super_segment_block in enumerate(ranking):
        for j, seg in enumerate(super_segment_block):
            temp_image[:, segments == seg] = base_color.repeat(1, np.sum(segments == seg))
            new_images[j] = temp_image

        new_images = new_images.reshape(-1, color_channels, width, height).double()
        new_probs[batch_size*i:batch_size*i+len(super_segment_block)] = calculate_probs(model, label, new_images)
        new_images = new_images.reshape(-1, color_channels, width*height).double()
...
def pixel_score(insert, model, image, attribution, label, pixel_group_size, batch_size):
    ...
    original_prob = calculate_probs(model, label, image.detach().reshape(-1, color_channels, width, height))[0]
    ...
    for j, pixel_super_block in enumerate(pixel_order):
        for k, pixel_block in enumerate(pixel_super_block):
            if insert:
                ...
                temp_image[:, pixel_block] = baseline.reshape(color_channels, 1).repeat(1, pixel_group_size)
            image_batch[k, :] = temp_image

        image_batch = image_batch.reshape(batch_size, color_channels, width, height)
        new_probs[j*batch_size:j*batch_size+len(pixel_super_block)] = \
