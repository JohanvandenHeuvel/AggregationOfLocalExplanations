...
def plot_explanations(explanations, titles, figsize, ):
...
def plot_orig_images(image, annotation=None):
    ...
    plt.subplot(1, 2, 1)
    plt.imshow(prepare_image_for_showing(
        image.cpu().detach().numpy()), cmap="gray")
...
def plot_rbm_weight_matrix(weight_matrix, methods):
    ...
    for i in range(len(methods)):
        ...
        plt.errorbar(x[i], y[i], std[i], fmt='',
                     label=methods[i], color=colors[i])
    if len(methods) < len(x):
        plt.plot(x[len(methods):], y[len(methods):],
                 marker='o', color=colors[-1])
        plt.errorbar(x[len(methods):], y[len(methods):], std[len(methods):],
                     fmt='', label="Random", color=colors[-1])
    plt.legend()
