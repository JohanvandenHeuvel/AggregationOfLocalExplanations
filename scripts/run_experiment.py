...
from .attribution import compute_explanations, compute_noise_attributions
...
def predict_label(model, images):
    # TODO what is this .double() doing
    # model = model.double()
    # output = model(torch.unsqueeze(image.double(), 0))
...
def task_consistency_check(tasks, ensemble_techniques):
    ...
    for task in tasks:
        ...
        assert (
            task["technique"] == "mean"
            or task["technique"] == "var"
            or task["technique"] == "rbm"
        )
...
def calculate_ensembles(title, ensemble_experiment_name, tasks, pos_filter):
    ...
    for file in tqdm(files):
        ...
        for i in range(len(ar.attributions)):
            noise_attributions = compute_noise_attributions(
                ar.images[i], nr_noise_experiments
            )
            ar.record_noise_attributions(noise_attributions)

            ensemble_attributions = compute_ensembles(
                ar.attributions[i], ar.noise_attributions[i], tasks, pos_filter
            )
...
def calc_statistics_package(
    ...
    if insert:
        insert_statistic = pixel_score(
            True, model, image, attribution, label, pixel_group_size, BATCH_SIZE
        )
    if delete:
        delete_statistic = pixel_score(
            False, model, image, attribution, label, pixel_group_size, BATCH_SIZE
        )
    if irof:
        irof_statistic = irof_score(
            model,
            image,
            attribution,
            raw_image,
...
def calculate_statistics(
    ...
    insert_score,
    delete_score,
    measure_individual_methods=True,
    measure_ensembles=True,
    ...
    for file in tqdm(files):
        ...
        if model is None:
            model = load_model(ar.dataset, ar.get_model).to(device)
        ar.create_statistics_table(
        ...
        for i in range(len(ar.attributions)):
            ...
            if measure_individual_methods:
                for j in range(len(ar.attributions[i])):
                    insert, delete, irof = calc_statistics_package(
                        model,
                        ar.images[i],
                        ar.raw_images[i],
                        ar.attributions[i][j],
                    ...
                    ar.record_attributions_statistics(
                        ar.methods[j], insert, delete, irof
                    )
            ...
            if measure_ensembles:
                for j in range(len(ar.ensemble_attributions[i])):
                    ...
                    insert_flip, delete_flip, irof_flip, prefer_flip = (
                        None,
                        None,
                        None,
                        False,
                    ...
                    if measure_flipped_rbms:
                        if ar.ensemble_tasks[j]["technique"] == "rbm":
                            attr_normalized = normalize_abs(
                                1 - ar.ensemble_attributions[i][j]
                            )
                            (
                                insert_flip,
                                delete_flip,
                                irof_flip,
                            ) = calc_statistics_package(
                                model,
                                ar.images[i],
                                ar.raw_images[i],
                                attr_normalized,
...
task2 = dict(
    {
        ...
        "nr_noise": 0,
        "rbm": [
            BernoulliRBM(n_components=1, batch_size=10, learning_rate=0.01, n_iter=100)
        ],
    }
)
